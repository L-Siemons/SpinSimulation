...
def _sinc_func(x, version='1'):
    ...
    if version == '1':
        ...
        return np.sinc(x)*np.sqrt(.1+1.24*x**2+1.46*x**4)*np.exp(-(x/2.)**2)
    elif version == '3':
...
def _gauss_func(x, version='1'):
    ...
    if version == '1':
        ...
        return (
            np.exp(-(2*x)**2)-0.51*(np.exp(-(x-1.33)**2)
            +np.exp(-(x+1.33)**2))+0.31*(np.exp(-(x-1.81)**2)+np.exp(-(x+1.81)**2))
        )
    else:
        raise NotImplementedError(
            f'Only version 1, 2, 3 are currently implemented, yours is {version}'
...
def sinc_pp_90(tau_p, phase=0, stretch=2, version='1', max_ampl=None, reversed=False):
    ...
    def _inner_func(ts):
        ...
        x_init = ts/tau_p       # normalize to [0, 1]
        x = x_init * 5     # normalize to [0, 5]
...
def gauss_pp_90(tau_p, phase=0, stretch=2, version='1', max_ampl=None, reversed=False):
    ...
    def _inner_func(ts):
        ...
        if version == '1':
            ...
            x_init = ts/tau_p       # scale to [0, 1]
            x = x_init * scaling    # scake to [0, 5]
            ...
            x_init = ts/tau_p       # scale to [0, 1]
            x = x_init * 5.6        # scake to [0, 5.6]
            x -= 0.6                # scale to [-0.6, 5]
        ampls = ampl_func(x) / max_ampl
        mask = (x == 0)
        ampls = np.where(mask, 0, ampls)
        phases = np.full(ampls.shape, 0, dtype=np.float64)
